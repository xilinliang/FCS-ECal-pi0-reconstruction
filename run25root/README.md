...
2) Put the final root file ({$run}/StFcsPi0invariantmassAll.root) under run25root/ folder, you need to rename the name so as they are not overwrite each other.

	Merge all the root files for 2 runs into one final root file. The final root file must name under: StFcsPi0invariantmass{$day}testAll_iteration{$N}.root  where {$day} is the period number + beginning (A) or ending (B) (for example "1A" is the beginning of period 1) , {$N} is number of iterations. Same as other step.
