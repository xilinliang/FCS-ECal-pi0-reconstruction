...
if ($yy eq $yy23) {print "$yy \n";
			$year = "2022";}
print "year = $year \n";
#my $mudstroot="/star/data*/reco/fcsGainCalibration//st_physics_*.MuDst.*";
#my $mudstroot="/star/data*/reco/fcsGainCalibration/ReversedFullField/dev/2022/$day/$run/st_physics_*.MuDst.*";
my $mudstroot="/star/data*/reco/production_pp500_2022/ReversedFullField/dev/2022/$day/$run/st_fwd_*.MuDst.*";
...
#$number_run= @file_run;
my $pi0root="/star/data01/pwg/liangxl/mywork/fcs_OO_Ecal/fcs_fastoffline/run22root/pT$run/";
...
#}
#	close(FILELIST);
	system("star-submit-template -template submitScheduler_dataevent_run22fcstest_pT.xml -entities runnumber=$run\n")==0
